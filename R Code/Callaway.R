...
using$estab_log <- log(using$estab_pop*100000)
using_local <- using
...
using_local <- using_local %>% select("id", "id_numeric","years", "YEAR.id","estab_pop","estab_log","receipt_estab",
...
# Med_diff_estabpop <-  att_gt(yname="estab_log",
#                              gname="first.treat.min_change",
#                              idname="id_numeric",
#                              tname="years",
#                              xformla=~quantile,
...
es_df$Treatment <- factor(es_df$Treatment, levels = c("No Change", "After Min. Wage Inc."))

myColors <- c("black","green3")
